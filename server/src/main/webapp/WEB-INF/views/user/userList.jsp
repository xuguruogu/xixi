	...
	<tbody>
		<c:forEach items="${userList}" var="user">
			<tr>
				...
				<td><c:if test="${user.status==0}">
						<a href="javascript: " class="tablelink"
							onclick="javascript:updateStatus(${user.id})">启用</a>
					</c:if> <c:if test="${user.status==1}">
						<a href="javascript: " class="tablelink"
							onclick="javascript:updateStatus(${user.id})">停用</a>
					</c:if></td>
				<td><a href="javascript: " class="tablelink"
					onclick="javascript:updateFunc(${user.id})">修改</a></td>
			</tr>
		</c:forEach>
	</tbody>
</table>
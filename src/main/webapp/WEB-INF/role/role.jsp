...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function gong(roleCount) {
			$("#pagination").pagination(roleCount, {
				first_text : "首页",
				prev_text : "上一页",
				next_text : "下一页",
				last_text : "尾页",
		...
		function pageselectCallback(page_id, jq) {
			...
			$.ajax( {
				type : "POST",
				url : "${bp}/role/roleList?pageIndex=" + page_id,
		...
		function addFunc(){
			window.location.href = "${bp}/role/toAdd";
		}
			...
			if(confirm("确认删除？")){
				$.ajax({
				     ...
				     success: function(msg){
					     ...
					     else if(msg=='no'){alert("删除失败，该角色已被用户拥有")}
					     else{alert("删除失败，请检查")}
						 window.location = "${bp}/role/role";
					}
				})
			}
		}
		...
		function details(id){
			var myart = {
				...
				artDialog.open("${bp}/role/details?roleID="+id, myart);
		}

		
		//修改功能信息
		function updateFunc(id){
			window.location.href = "${bp}/role/toUpdate?roleID="+id;
		}

		function search(){
			$("#search_form").attr("action","${bp}/role/role");
			$("#search_form").submit();
		}
	</script>

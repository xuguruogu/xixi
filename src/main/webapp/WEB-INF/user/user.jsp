  ...
  <head>
	...
	function gong(userCount) {
		...
		function pageselectCallback(page_id, jq) {
			...
			$.ajax( {
				type : "POST",
				url : "${bp}/user/userList?pageIndex=" + page_id,
		...
		function addFunc(){
			var myart = {
					...
					if(flag==1){
						$.ajax({
						     type: "POST",
						     url: "${bp}/user/userList?pageIndex=" + tempPage,
						     data:$('#search_form').serialize(),
						 	 dataType : "text",
						     success: function(msg){
						    	 $("#manger").html(msg);
			...
			artDialog.open("${bp}/user/toAdd", myart);
		}
		...
		function updateStatus(id){
			if(confirm("确认更改状态？")){
				$.ajax({
				     type: "POST",
				     url: "${bp}/user/updateStatus?id="+id,
				 	 dataType : "text",
				     success: function(msg){
					     if(msg=='yes') {alert("更改成功")}
					     else{alert("更改失败")}
					     gong('${userCount}');
					}
				})
	...
	function updateFunc(id){
		...
		artDialog.open("${bp}/user/toUpdate?id="+id, myart);
	}

	function search(){
		$("#search_form").attr("action","${bp}/user/user");
		$("#search_form").submit();
	}
	</script>

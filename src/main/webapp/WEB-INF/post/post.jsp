  ...
  <head>
	...
	<script type="text/javascript">
		...
		function pageselectCallback(page_id, jq) {
			...
			$.ajax( {
				type : "POST",
				url : "${bp}/post/postList?pageIndex=" + page_id,
	...
	function topFunc(id,orderNum){
		...
		if(confirm(question)){
			$.ajax({
				type: 'GET',
				url: '${bp}/post/updateTop?id='+id+'&orderNum='+orderNum,
				success: function(msg){
					if(msg=='yes'){
						if(orderNum==0){
							alert("置顶成功");
						...
						$.ajax( {
							type : "POST",
							url : "${bp}/post/postList?pageIndex=" + tempPage,
							data : $("#search_form").serialize(),
							success : function(msg) {
								$("#manger").html(msg);
								$('.tablelist tbody tr:odd').addClass('odd');
	...
	/*
		...
		*/
		...
		function lookPost(id){
			...
			artDialog.open("${bp}/post/lookpost_admin?postID="+id, myart);
		}
		/*
		//修改功能信息
		...
		*/
		...
		//搜索
		function search(){
			$("#search_form").attr("action","${bp}/post/post");
			$("#search_form").submit();
		}
	</script>
